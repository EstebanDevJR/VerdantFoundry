...
function AgentList({ onEdit }: { onEdit: () => void; key?: string }) {
  const { addNotification } = useStore();
  const agents = [
    { id: 'alpha-7', name: 'Alpha-7', role: 'Deep Search', version: 'v2.4.1', status: 'Active' },
    { id: 'beta-2', name: 'Beta-2', role: 'Synthesis & Analysis', version: 'v1.9.0', status: 'Idle' },
  ];

  const handleHistory = (agentName: string) => {
    addNotification({
      title: "Agent History",
      message: `Viewing version history for ${agentName}.`,
      type: "info"
    });
  };
        ...
        {agents.map(agent => (
          <div key={agent.id} className="p-5 bg-white/60 border border-slate-200 rounded-2xl hover:border-primary/30 hover:shadow-md transition-all group">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="px-2 py-1 rounded-md bg-slate-100 text-slate-600 text-xs font-mono">{agent.version}</span>
            </div>
...
function ToolList({ onEdit }: { onEdit: () => void; key?: string }) {
  const { addNotification } = useStore();
  const tools = [
    { id: 'web-search', name: 'Web Search API', type: 'External', version: 'v1.2.0' },
    { id: 'db-query', name: 'Vector DB Query', type: 'Internal', version: 'v3.0.1' },
  ];

  const handleHistory = (toolName: string) => {
    addNotification({
      title: "Tool History",
      message: `Viewing version history for ${toolName}.`,
      type: "info"
    });
  };
        ...
        {tools.map(tool => (
          <div key={tool.id} className="p-5 bg-white/60 border border-slate-200 rounded-2xl hover:border-primary/30 hover:shadow-md transition-all group">
            <div className="flex justify-between items-start mb-4">
              ...
              <span className="px-2 py-1 rounded-md bg-slate-100 text-slate-600 text-xs font-mono">{tool.version}</span>
            </div>
            <div className="flex gap-2">
              <button onClick={onEdit} className="flex-1 py-2 rounded-lg bg-slate-50 text-slate-600 text-sm font-medium hover:bg-slate-100 transition-colors flex items-center justify-center gap-2">
...
function AgentEditor({ onCancel }: { onCancel: () => void; key?: string }) {
  const { addNotification } = useStore();
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      ...
      addNotification({
        title: "Agent Saved",
        message: "Agent configuration has been saved and deployed.",
        type: "success"
      });
      onCancel();
    }, 1000);
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div>
                ...
                <input type="text" defaultValue="Delta-X" className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-primary/50 outline-none transition-all" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Primary Role</label>
                <input type="text" defaultValue="Data Synthesis" className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-primary/50 outline-none transition-all" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Core Objective</label>
                <textarea rows={4} defaultValue="Analyze and synthesize large datasets into actionable insights." className="w-full px-4 py-2.5 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-primary/50 outline-none transition-all resize-none" />
              </div>
            </motion.div>
          )}
          ...
          {step === 2 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
              ...
              <div className="space-y-3">
                {['Web Search API', 'Vector DB Query', 'Code Execution', 'File System Access'].map((tool, i) => (
                  <label key={i} className="flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:bg-slate-50 cursor-pointer transition-colors">
                    <div className="flex items-center gap-3">
                      <input type="checkbox" defaultChecked={i < 2} className="w-4 h-4 rounded text-primary focus:ring-primary/50" />
...
function ToolEditor({ onCancel }: { onCancel: () => void; key?: string }) {
  return (
